...
import { ProcessLogger, ProcessStatus, ReadyCheck } from './types'
...
export class ManagedProcess {
  ...
  private _onExitCallbacks: ((code: number | null, signal: NodeJS.Signals | null) => void)[] = []
  ...
  constructor(command: string, args: string[], readyChecks: ReadyCheck[], logger: ProcessLogger) {
  ...
  start(): void {
    ...
    this._process.on('exit', (code, signal) => {
      ...
      if (this._status === 'completed' && this._checksPassed === this.readyChecks.length) {
        this._readyResolve()
      }
  ...
  stop(timeout: number = 1000, signal: StopSignal = 'SIGINT'): void {
    if (!this.isRunning()) {
      ...
      console.warn(`Process: ${this.command} is not running. Status: ${this._status}.`)
    }
  ...
  prepareForRestart(): void {
    if (this.isRunning() || this._status === 'stopping') {
      throw new Error('Cannot prepare for restart while process is running or stopping')
    }
