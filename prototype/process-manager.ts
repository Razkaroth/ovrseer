...
export class ProcessManager implements ProcessManagerI {
  ...
  constructor(options?: ProcessManagerOptions) {
    ...
    this.tui = options?.tui ?? new TUIRenderer()
  }
  ...
  stop(): void {
    ...
    ;(async () => {
      ...
      for (const [id, cleanup] of this.cleanupProcesses) {
        ...
        try {
          await this.waitForPromiseWithTimeout(cleanup.finished, this.cleanupTimeout, id)
        } catch (e: any) {
          this.tui?.showStatus?.(`Cleanup ${id} timeout: ${e?.message || 'timeout'}`)
        }
      }
  ...
  restartProcess(id: string, processType?: TUIProcessType): void {
    ...
    if (processType === 'dependency') {
      ...
      process =
        this.mainProcesses.get(id) || this.dependencies.get(id) || this.cleanupProcesses.get(id)
    }
  ...
  restartAll(): void {
    ...
    const anyRunning = [...this.dependencies.values(), ...this.mainProcesses.values()].some((p) =>
      p.isRunning(),
    )
    if (!anyRunning) {
    ...
    for (const entry of this.dependencies) if (entry[1].isRunning()) runningDeps.push(entry)
    for (const entry of this.mainProcesses) if (entry[1].isRunning()) runningMain.push(entry)
    ...
    ;(async () => {
      ...
      if (runningDeps.length) {
        try {
          ...
          this.tui?.showStatus?.(`Dependency failed after restart: ${e?.message || e}`)
          return
        }
      }
  ...
  startTuiSession(): void {
    ...
    this.tui.onKeyPress((key: string, meta?: import('./types').TUIKeyPressMeta) => {
      if (key === 'q' || key === 'C-c') {
        ...
        if (this.tuiState.selectedProcessId && this.tuiState.selectedProcessType) {
          this.restartProcess(this.tuiState.selectedProcessId, this.tuiState.selectedProcessType)
        }
  ...
  private setupProcessHandlers(id: string, process: ManagedProcessI, type: TUIProcessType): void {
    process.onCrash((err) => {
      this.handleCrash(id, process, type, err)
    })

    process.onExit((code, signal) => {
      this.tui?.showStatus?.(`Process ${id} exited with code ${code}, signal ${signal}`)
      this.updateTui()
    })
    ...
    process.logger.onLog(() => {
      this.updateTui()
      if (this.tuiState.selectedProcessId === id && this.tuiState.selectedProcessType === type) {
        this.updateLiveLogsForSelectedProcess()
      }
    })
  ...
  private handleCrash(
    ...
    if (type === 'dependency') {
      ...
      if (this.crashReporter) {
        const report = this.crashReporter.generateReport(id, process, 'dependencyFailed', {
          error,
          processType: type,
        })
    ...
    if (currentRetries < this.maxRetries) {
      ...
      if (this.crashReporter) {
        const report = this.crashReporter.generateReport(id, process, 'maxRetriesExceeded', {
          error,
          processType: type,
  ...
  private getProcessByIdAndType(id: string, type: TUIProcessType): ManagedProcessI | undefined {
  ...
  private updateLiveLogsForSelectedProcess(): void {
    if (!this.tuiState.selectedProcessId || !this.tuiState.selectedProcessType) return
    const process = this.getProcessByIdAndType(
      this.tuiState.selectedProcessId,
      this.tuiState.selectedProcessType,
    )
    if (process) {
      ...
      this.tui?.showLogs?.(this.tuiState.selectedProcessId, this.tuiState.selectedProcessType, logs)
    }
  }
