...
import { ProcessLogger } from './types'
...
export class SimpleLogger implements ProcessLogger {
  ...
  constructor(maxBufferSize: number, maxLogSize: number, defaultSeparator?: string) {
  ...
  public getLogs(options?: { index?: number; numberOfLines?: number; separator?: string }) {
    const {
      index = 0,
